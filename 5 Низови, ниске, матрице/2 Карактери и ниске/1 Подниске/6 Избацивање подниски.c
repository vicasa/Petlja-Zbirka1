	...
	while(flag){
		...
		for(int i=0; i<br_reci; i++){
			...
			while(indeks!=-1){
				...
				indeks=podniska(s+pomoc,reci[i], l); /*indeks koji vrati ovaj poziv funkcije podniska je indeks racunajuci od indeksa pomoc podniske s, zato kasnije njega sabiramo sa pomoc da bi dobili indeks niske s racunajuci od 0*/
				if(indeks != -1)
					indeks += pomoc;
			}
		}
